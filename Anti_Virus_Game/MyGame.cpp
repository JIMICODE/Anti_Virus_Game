...
void Update_Background()
{
	...
	if (scrollx > BUFFERW - SCREENW)
		scrollx = 0;
	//update virtual scroll posiotion
	virtual_scrollx += 1.0;
...
void Update_Bullets()
{
		...
		&& player_state_previous == OVERLOADING)
	{
		...
		for (int n = 0 ; n < BULLETS; ++n)
		{
			//overload bullets start with zero velocity
...
void Update_Viruses()
{
	...
	for (int n = 0; n < VIRUSES; ++n)
	{
		if (viruses[n].alive)
		{
			//move hroiz based on x velocity
...
void Test_Virus_Collision()
{
	...
	for (int v = 0; v < VIRUSES; ++v)
	{
		if (viruses[v].alive)
		{
			//test coolision with every live bullet
...
void Draw_Overloading_Ship()
{
	for (int n = 0; n < 4; ++n)
	{
		D3DCOLOR overcolor = D3DCOLOR_ARGB(
			150 + rand()%100, 80, 255, 255);
...
void Draw_Player_Shadows()
{
	...
	if (currenttime > position_history_timer + 40)
	{
		...
		if (position_history_index > 7)
		{
			...
			for (int a = 1; a < 8; ++a)
				position_history[a - 1] = position_history[a - 1];
		}
	...
	for (int n = 0; n < 8; ++n)
	{
		...
		//draw shadows of prevoius ship position
		Sprite_Transfrom_Draw(
			player_ship,
...
void Draw_HUD()
{
	...
	FontPrint(font, "SCORE " + ToString(score), 900, 0, color);
	FontPrint(font, "LIVES " + ToString(lives), 10, 0, color);
	//draw debug messages
	FontPrint(debugfont, "", 0, y, debugcolor);
	FontPrint(debugfont, "Core FPS = " + ToString(corefps) + "(" + ToString(1000.0 / corefps) + "ms)",
		0, y - 12, debugcolor);
	FontPrint(debugfont, "Screen FPS = " + ToString(screenfps), 0, y - 24, debugcolor);
	FontPrint(debugfont, "Ship X,Y = " + ToString(player.x) + "," + ToString(player.y),
		0, y - 36, debugcolor);
	FontPrint(debugfont, "Bullets = " + ToString(bulletcount), 0, y - 48, debugcolor);
	FontPrint(debugfont, "Buffer Scroll = " + ToString(scrollx), 0, y - 60, debugcolor);
	FontPrint(debugfont, "Virtual Scroll = " + ToString(virtual_scrollx) + "/" + ToString(virtual_level_size),
		0, y - 72, debugcolor);
	FontPrint(debugfont, "Fragment[0] = " + ToString(fragments[0].x) + "," + ToString(fragments[0].y),
		0, y - 84, debugcolor);
}
...
void Draw_Mission_Briefing()
{
	...
	{
		FontPrint(font, briefing[line], 52, y + 2, black);
		FontPrint(font, briefing[line], 50, y, white);
		y += 20;
	}
	...
	for (int line = 0; line < array_size; ++line)
	{
		FontPrint(font, controls[line], x + 2, y + 2, black);
		FontPrint(font, controls[line], x, y,green);
		y += 60;
	}
}
...
void Game_Run(HWND window)
{
	...
	if (currenttime > refresh + 16)
	{
		...
		{
			player_state = NORMAL;
			if (Key_Down(DIK_UP) || Key_Down(DIK_W) || controllers[0].sThumbLY > 2000)
				move_player(0, -1);
			if (Key_Down(DIK_DOWN) || Key_Down(DIK_S) || controllers[0].sThumbLY < -2000)
				move_player(0, 1);
			if (Key_Down(DIK_LEFT) || Key_Down(DIK_A) || controllers[0].sThumbLX < -2000)
				move_player(-1, 0);
			if (Key_Down(DIK_RIGHT) || Key_Down(DIK_D) || controllers[0].sThumbLX > 2000)
				move_player(1, 0);
			if (Key_Down(DIK_LCONTROL) || controllers[0].wButtons & XINPUT_GAMEPAD_B)
				player_state = PHASING;
			if (Key_Down(DIK_LSHIFT) || controllers[0].wButtons & XINPUT_GAMEPAD_Y)
			{
				if (!player_overload())
					player_state_previous = OVERLOADING;
				else
			...
			if (Key_Down(DIK_SPACE) || controllers[0].wButtons & XINPUT_GAMEPAD_A)
				player_shoot();
			Update_Background();
			Update_Bullets();
			Update_Viruses();
			Update_Fragments();
			Test_Virus_Collision();
